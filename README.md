...
一个原生 Android App + 一个轻量 Node 桥（cc-bridge），把你自己机器上跑的
[Claude Code](https://claude.com/claude-code) 变成手机里的聊天应用——
**用你的订阅额度，不烧 API key**。

它解决两件事：

1. **claude.ai 的对话不再说没就没。** 网页端的对话偶尔会被突然锁死，再也点不开。
   把官方导出的数据丢给 Telos，旧对话就**原样导入**你自己的 Claude Code 里无缝续聊——
   一样的聊天气泡、一样的上下文，但对话从此是**你自己磁盘上的文件**，谁也收不走。
2. **手机随时遥控自己的 cc。** 流式回复、工具卡片、权限弹窗、文件互传，
   体验对齐官方移动端，干活的是你 VPS / 家里机器上登录好的 `claude`。

<!-- TODO: 放 2~3 张截图（会话列表 / 聊天流式 / 导入选择页）到这里 -->

## 是什么 / 不是什么

- **是**：你自己 cc 的手机遥控器 + 你的 Claude 对话的「家」。
- **不是**：云服务。App 不含任何账号和模型，只是「连到某个 bridge 的地址 + token」。
  用谁的额度、对话存在哪，完全取决于 bridge 跑在哪台机器。

## 功能

- 读取并续聊 cc 已有会话、新建会话、编辑/重生成（fork）
- 流式逐字回复、工具调用紧凑卡片、思考块、权限弹窗（允许/拒绝）
- **导入 claude.ai 导出的对话，无缝续聊**（见下）
- **克隆为全新窗口**：把一个对话原样复制成另一个独立会话，旧的原地不动
- 选中回复里的文字即浮现工具条：复制 / 引用到输入框 / **收藏**（独立收藏夹）/ 系统分享
- 模型切换 + 1M 上下文识别、账号用量 / 累计花费 / 活跃天数统计
- 文件上传 / 下载、图片与音频预览、按路径把附件交给 cc
- 会话置顶 / 文件夹分类 / 全文搜索 / 自动清理废弃对话
- 多主题换肤 + 强调色、手势导航、可调触感反馈
...
3. 长按这个文件 → **导入为对话** → 勾选想要的对话 → 导入；
4. 每个对话变成一个标准的 cc 会话（存在 `~/.claude` 下），点开直接接着聊——
   模型看得到全部历史上下文，体验和原来一样，但这次对话归你自己保管。
...
- **导入的对话会烧 API 费用吗？** 不会。续聊跑的是你机器上登录的 `claude`，走订阅额度。
