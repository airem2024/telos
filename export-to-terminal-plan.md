...
实战暴露的坑：App 续写终端会话时，SDK 给新行盖 `entrypoint:"sdk-ts"` 印 → 整条会话被 `/resume`
过滤掉、从终端「消失」→ 用户被迫一遍遍「转到终端」，同一对话攒出三个副本。三项修正（纯服务端）：

1. **洗印（detoxEntrypoints）**：每轮结束后，终端出身的会话（首个 entrypoint=cli）把混入的
   sdk 印洗回 `cli`。SDK 尾行在 result 之后才落盘，所以延迟数秒执行 + 下轮开始前补洗兜底。
   自此同一条会话手机/终端轮流聊，永远留在 `/resume`。Telos 原生会话（首印 sdk-ts）不动。
2. **护栏**：对已是终端出身的会话再点「转到终端」→ 拒绝克隆（防副本增殖），顺手洗印。
3. **移交语义**：藏「过时的原版」而不是克隆（最初上线时是反的）——克隆配合洗印才是双端都能
   接着聊的那份，必须留在列表里。模型偏好/文件夹/置顶跟着克隆走；原版的唤醒排程就地停掉。
   服务端补发 `{type:'deleted'}` 让老客户端刷新列表，零前端改动。
